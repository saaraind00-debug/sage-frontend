...
const MOOD_ICONS = {
  Happy: "😊", Anxious: "😰", Tired: "😴",
  Motivated: "🔥", Sad: "😔", Grateful: "🙏",
  Overwhelmed: "😵", Calm: "🌊",
};

const MOOD_COLORS = {
  Happy: "#f5c518", Anxious: "#e67e22", Tired: "#9b59b6",
  Motivated: "#e74c3c", Sad: "#3498db", Grateful: "#27ae60",
  Overwhelmed: "#e74c3c", Calm: "#1abc9c",
};
...
window.sendMessage = async function () {
  ...
  if (currentMood) {
    const moodEntry = { mood: currentMood, timestamp: new Date().toISOString() };
    moodLog.unshift(moodEntry);
    // Clear selection
    document.querySelectorAll(".mood-chip").forEach((c) => c.classList.remove("selected"));
    currentMood = null;
  }
  ...
  try {
    const response = await fetch(`${API_BASE}/chat`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ sessionId, message: text, mood: currentMood }),
    });
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        if (payload.type === "error") {
          if (!replyEl) replyEl = createStreamingBubble();
          replyEl.textContent = payload.message;
        }
      }
    }
  } catch (err) {
...
function renderJournalPrompts() {
  ...
  JOURNAL_PROMPTS.forEach((p) => {
    ...
    card.onclick = () => {
      showTab("chat");
      injectPrompt(`I'd like to reflect on this: "${p.prompt}"`);
    };
...
function renderMarkdown(text) {
  // Simple inline markdown parser (no external dependency needed for basic formatting)
  return text
